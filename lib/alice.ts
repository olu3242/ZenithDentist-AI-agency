...
export async function answerOperationalQuery(question: string): Promise<AliceFrameworkResponse> {
  ...
  const [portalData] = await Promise.all([getPortalData(tenantData.tenant.organizationId)]);
...
export async function coordinateEnterpriseIntelligence(
  ...
  const [cloud, tenantDataForPortal] = await Promise.all([getEnterpriseCloudState(), getTenantData()]);
  const portalData = await getPortalData(tenantDataForPortal.tenant.organizationId);
  const latest = portalData.metrics[0];
