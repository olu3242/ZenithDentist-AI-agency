...
export default async function PMSSyncHealthPage() {
  const [state, tenantData, role] = await Promise.all([getPMSOperationsState(), getTenantData(), getCurrentZenithRole("staff")]);
  return <AppShell role={role} organization={tenantData.organization} locations={tenantData.locations}><PMSOperationsCenter state={state} section="sync-health" /></AppShell>;
}
