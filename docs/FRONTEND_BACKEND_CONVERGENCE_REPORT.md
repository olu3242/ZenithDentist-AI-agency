...
| 6 | Analytics visible in Executive Dashboard | PASS | `lib/mission-control/index.ts:14,35,68` includes `workflowAnalytics` and `workflowHealth` in `MissionControlState`; both are returned by `getMissionControlState()` and consumed by `app/api/mission-control/state/route.ts` | None |
