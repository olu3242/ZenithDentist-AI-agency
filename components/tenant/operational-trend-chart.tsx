...
export function OperationalTrendChart({ metrics, field, label }: { metrics: OperationalMetric[]; field: keyof OperationalMetric; label: string }) {
  ...
  return (
    <section className="rounded border border-line bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 grid h-52 grid-cols-8 items-end gap-2 border-b border-line">
        {data.map(item => {
          const value = Number(item[field] ?? 0);
          return (
            <div key={`${item.id}-${String(field)}`} className="relative rounded-t bg-teal" style={{ height: `${Math.max(8, (value / max) * 190)}px` }}>
