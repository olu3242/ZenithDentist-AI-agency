...
export function OnboardingProgress({ organization }: { organization: Organization }) {
  ...
  return (
    <section className="rounded border border-line bg-white p-6 shadow-sm">
      <p className="text-xs font-black uppercase tracking-wider text-teal">Consultative onboarding</p>
      <h2 className="mt-2 text-2xl font-black">Practice operating model setup</h2>
      <div className="mt-6 grid gap-3">
        {steps.map(([key, label], index) => {
          const complete = index <= activeIndex;
          return (
            <div key={key} className="flex items-center gap-3 rounded bg-paper p-4">
              <span className={`grid h-8 w-8 place-items-center rounded-full text-sm font-black ${complete ? "bg-teal text-white" : "bg-white text-muted"}`}>{index + 1}</span>
              <div>
                <strong>{label}</strong>
