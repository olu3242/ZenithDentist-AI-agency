  ...
  return (
    <section className="grid gap-4 lg:grid-cols-3">
      {plans.map(plan => {
        ...
        return (
          <article key={plan.id} className={`rounded border bg-white p-5 shadow-sm ${plan.plan_key === activePlan ? "border-teal" : "border-card"}`}>
            ...
            <strong className="mt-3 block text-3xl text-accent">{formatCurrency(plan.price_monthly)}<span className="text-sm text-muted">/mo</span></strong>
