    ...
    <section className="rounded border border-line bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 grid gap-3">
        {locations.map((location, index) => {
          ...
          return (
            <div key={location.id} className="grid gap-3 rounded bg-paper p-4 md:grid-cols-5">
              ...
              <span className="font-bold text-green">{formatCurrency(Number(metric?.recovered_revenue ?? 0))}</span>
            </div>
          );
        })}
      </div>
