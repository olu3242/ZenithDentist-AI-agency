...
export function SlaBreachPanel({ state }: { state: RuntimeHealthState }) {
  return (
    <section className="rounded border border-line bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-ink">Workflows exceeding runtime thresholds</h2>
      <div className="mt-5 grid gap-3">
        {state.slaBreaches.length ? state.slaBreaches.map(trace => (
          <div key={trace.trace_id} className="rounded border border-line bg-paper p-4">
          ...
          <div className="rounded border border-line bg-paper p-4 text-sm font-semibold text-muted">No SLA breaches detected in live runtime data.</div>
        )}
      </div>
    </section>
  );
