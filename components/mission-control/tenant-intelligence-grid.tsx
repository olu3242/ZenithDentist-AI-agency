...
export function TenantIntelligenceGrid({ tenant }: { tenant: TenantIntelligenceState }) {
  return (
    <section className="rounded border border-line bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-ink">{tenant.organizationName}</h2>
        ...
        {tenant.benchmarkPercentiles.map(item => (
          <div key={item.label} className="rounded border border-line bg-paper p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-xl font-black text-teal">P{item.percentile}</span>
            </div>
      ...
      <div className="mt-5 grid gap-2">
        {tenant.recommendations.map(item => (
          <div key={item} className="rounded border border-line bg-white p-3 text-sm font-semibold text-muted">{item}</div>
        ))}
      </div>
    </section>
  );
