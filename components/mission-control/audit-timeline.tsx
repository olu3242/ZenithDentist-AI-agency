...
export function AuditTimeline({ governance }: { governance: GovernanceState }) {
  ...
  return (
    <section className="rounded border border-line bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-ink">Governance history</h2>
      <div className="mt-5 border-l border-line pl-4">
        {derived.map(item => (
          <div key={item.id} className="mb-5 last:mb-0">
            <p className="text-xs font-black uppercase tracking-wider text-teal">{item.severity}</p>
            <strong className="mt-1 block text-sm font-black text-ink">{item.title}</strong>
