...
export function AutomationDomainMatrix({ state }: { state: AutomationAuditState }) {
  return (
    <section className="rounded border border-line bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-ink">End-to-end operational coverage by domain</h2>
      <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        {state.domainCoverage.map(domain => (
          <div key={domain.domain} className="rounded border border-line bg-paper p-4">
            <strong className="block text-sm font-black text-ink">{domain.domain.replace(/_/g, " ")}</strong>
            <p className="mt-3 text-3xl font-black text-teal">{domain.score}%</p>
            ...
            <div className="mt-3 h-2 rounded-full bg-line">
              <div className="h-2 rounded-full bg-teal" style={{ width: `${domain.score}%` }} />
            </div>
          </div>
        ))}
      </div>
