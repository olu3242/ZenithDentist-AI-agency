...
export function ReplayConsole({ state }: { state: MissionControlState }) {
  return (
    <section className="rounded border border-card bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-[#F8FAFC]">Replay requests and failed event recovery</h2>
      <div className="mt-5 grid gap-3 lg:grid-cols-2">
        {state.replayEvents.map(event => (
          <article key={event.id} className="rounded border border-card bg-background p-4">
            <div className="flex items-center justify-between gap-3">
              <strong className="text-sm font-black text-[#F8FAFC]">{event.replay_scope.replace(/_/g, " ")}</strong>
              <span className="rounded-full bg-white px-2 py-1 text-xs font-black uppercase text-accent">{event.status}</span>
            </div>
