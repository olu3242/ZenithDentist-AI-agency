...
export function SimulationAccuracyChart({ state }: { state: MissionControlState }) {
  return (
    <section className="rounded border border-card bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-[#F8FAFC]">Projection reliability from forecast validation</h2>
        ...
        {state.forecastAccuracy.map(item => (
          <div key={item.id} className="rounded border border-card bg-background p-4">
            ...
            <div className="mt-3 h-2 rounded-full bg-line">
              <div className="h-2 rounded-full bg-success" style={{ width: `${Math.round(item.quality_score * 100)}%` }} />
            </div>
          </div>
        ))}
      </div>
