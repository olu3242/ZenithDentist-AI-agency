...
export function ForecastDriftRadar({ state }: { state: MissionControlState }) {
  return (
    <section className="rounded border border-card bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-[#F8FAFC]">Projected vs actual outcome drift</h2>
        ...
        {state.forecastAccuracy.map(item => (
          <article key={item.id} className="rounded border border-card bg-background p-4">
            <div className="flex items-center justify-between gap-3">
              <strong className="text-sm font-black text-[#F8FAFC]">{item.forecast_type.replace(/_/g, " ")}</strong>
              <span className="text-xl font-black text-danger">{Math.round(item.drift_score * 100)}%</span>
            </div>
            ...
            <div className="mt-4 h-2 rounded-full bg-line">
              <div className="h-2 rounded-full bg-danger" style={{ width: `${Math.round(item.drift_score * 100)}%` }} />
            </div>
          </article>
        ))}
      </div>
