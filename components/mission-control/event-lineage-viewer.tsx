...
export function EventLineageViewer({ state }: { state: MissionControlState }) {
  return (
    <section className="rounded border border-line bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-ink">Recommendation origins and supporting signals</h2>
        ...
        {state.recommendationLineage.map(item => (
          <article key={item.id} className="rounded border border-line bg-paper p-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <strong className="text-sm font-black text-ink">{item.expected_outcome}</strong>
