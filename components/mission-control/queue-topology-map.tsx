...
export function QueueTopologyMap({ state }: { state: MissionControlState }) {
  ...
  return (
    <section className="rounded border border-line bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-ink">Pipeline dependencies and pressure</h2>
      <div className="mt-5 grid gap-3 md:grid-cols-3 xl:grid-cols-6">
        {pipelines.map(pipeline => {
          const events = state.queueEvents.filter(event => event.pipeline === pipeline);
          return (
            <div key={pipeline} className="rounded border border-line bg-paper p-4">
              <strong className="text-sm font-black text-ink">{pipeline}</strong>
              <p className="mt-2 text-3xl font-black text-teal">{events.length}</p>
