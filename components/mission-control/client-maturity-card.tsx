export function ClientMaturityCard({ title, score, detail }: { title: string; score: number; detail: string }) {
  return (
    <div className="rounded border border-card bg-white p-5 shadow-sm">
      <p className="text-xs font-black uppercase tracking-wider text-muted">{title}</p>
      <div className="mt-4 flex items-end justify-between gap-4">
        <strong className="text-4xl font-black text-[#F8FAFC]">{score}%</strong>
        <span className={score >= 80 ? "text-xs font-black uppercase text-success" : score >= 55 ? "text-xs font-black uppercase text-warning" : "text-xs font-black uppercase text-danger"}>
