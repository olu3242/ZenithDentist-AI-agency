...
export function IntelligenceRunViewer({ state }: { state: MissionControlState }) {
  return (
    <section className="rounded border border-card bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-[#F8FAFC]">Evaluation runs and grounding sources</h2>
      <div className="mt-5 grid gap-3">
        {state.intelligenceRuns.map(run => (
          <article key={run.id} className="rounded border border-card bg-background p-4">
            <div className="flex items-center justify-between gap-3">
              <strong className="text-sm font-black text-[#F8FAFC]">{run.run_type.replace(/_/g, " ")}</strong>
              <span className="rounded-full bg-white px-2 py-1 text-xs font-black uppercase text-accent">{run.status}</span>
            </div>
