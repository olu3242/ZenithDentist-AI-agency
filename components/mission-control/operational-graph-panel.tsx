    ...
    <section className="rounded border border-line bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="text-2xl font-black text-ink">Workflow dependency map</h2>
        </div>
        <span className="rounded-full bg-rust/10 px-3 py-1 text-xs font-black text-rust">{graph.operationalRisk}/100 risk</span>
      </div>
        ...
        {topNodes.map(node => (
          <div key={node.id} className="rounded border border-line bg-paper p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-sm font-black uppercase text-teal">{node.type.replace(/_/g, " ")}</span>
            </div>
