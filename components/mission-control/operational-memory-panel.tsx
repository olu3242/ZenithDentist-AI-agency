...
export function OperationalMemoryPanel({ memory }: { memory: OperationalMemoryState }) {
  return (
    <section className="rounded border border-line bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-ink">Recurring intelligence signals</h2>
      <div className="mt-5 grid gap-3">
        {[...memory.candidates, ...memory.persisted.map(item => ({
          ...
          <div key={`${item.memoryType}-${item.title}`} className="rounded border border-line bg-paper p-4">
            <div className="flex items-center justify-between gap-3">
              <strong className="text-sm font-black text-ink">{item.title}</strong>
              <span className="text-xs font-black text-teal">{item.confidence}%</span>
            </div>
            <p className="mt-2 text-sm font-semibold text-muted">{item.summary}</p>
          </div>
        ))}
        {!memory.candidates.length && !memory.persisted.length ? (
          <div className="rounded border border-line bg-paper p-4 text-sm font-semibold text-muted">No recurring runtime memory signals are present.</div>
        ) : null}
      </div>
    </section>
  );
