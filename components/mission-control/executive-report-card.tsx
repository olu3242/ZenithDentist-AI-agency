...
export function ExecutiveReportCard({ report }: { report: ExecutiveReportSnapshot }) {
  return (
    <section className="rounded border border-card bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-[#F8FAFC]">{report.title}</h2>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {report.exportFormats.map(format => (
          <span key={format} className="rounded-full bg-accent/10 px-3 py-1 text-xs font-black text-accent">{format.replace(/_/g, " ")}</span>
        ))}
      </div>
    </section>
  );
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded border border-card bg-background p-3">
      ...
      <strong className="mt-1 block text-xl font-black text-[#F8FAFC]">{value}</strong>
    </div>
  );
}
