    ...
    <section className="rounded border border-card bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-black text-[#F8FAFC]">Mitigation sequencing</h2>
        </div>
        <span className="rounded-full bg-accent/10 px-3 py-1 text-xs font-black text-accent">{orchestrator.stabilizationScore}% stabilization</span>
      </div>
      <div className="mt-5 grid gap-3">
        {orchestrator.plans.slice(0, 6).map(plan => (
          <div key={plan.orchestrationKey} className="rounded border border-card bg-background p-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <strong className="text-sm font-black text-[#F8FAFC]">{plan.title}</strong>
              <span className={plan.status === "blocked" ? "text-xs font-black uppercase text-danger" : plan.status === "approval_required" ? "text-xs font-black uppercase text-warning" : "text-xs font-black uppercase text-success"}>{plan.status.replace(/_/g, " ")}</span>
            </div>
