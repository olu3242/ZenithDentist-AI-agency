...
export function OperationalMarketplace({ state }: { state: ProductizationState }) {
  return (
    <section className="rounded border border-card bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 text-2xl font-black text-[#F8FAFC]">Extension framework</h2>
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        {state.extensions.map(extension => (
          <div key={extension.key} className="rounded border border-card bg-background p-4">
            <div className="flex items-center justify-between gap-3">
              <strong className="text-sm font-black text-[#F8FAFC]">{extension.name}</strong>
              <span className="text-xs font-black uppercase text-accent">{extension.status}</span>
            </div>
