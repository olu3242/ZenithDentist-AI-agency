      ...
      <svg className="mt-6 h-56 w-full overflow-visible" viewBox="0 0 400 220" role="img" aria-label="Recovered revenue trend chart">
        <defs>
          <linearGradient id="trendFill" x1="0" x2="0" y1="0" y2="1">
            <stop offset="0%" stopColor={brandConfig.colors.secondary} stopOpacity="0.28" />
            <stop offset="100%" stopColor={brandConfig.colors.secondary} stopOpacity="0" />
          </linearGradient>
        </defs>
        <polyline points={`20,190 ${points} 380,190`} fill="url(#trendFill)" stroke="none" />
        <polyline points={points} fill="none" stroke={brandConfig.colors.secondary} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
        {data.map((item, index) => {
          const x = 20 + index * (360 / Math.max(1, data.length - 1));
          const y = 180 - (Number(item.recovered_revenue) / max) * 145;
          return <circle key={item.id} cx={x} cy={y} r="5" fill={brandConfig.colors.secondary} />;
        })}
      </svg>
    </section>
  );
