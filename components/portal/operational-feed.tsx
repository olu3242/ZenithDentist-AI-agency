    ...
    <section className="rounded border border-card bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 grid gap-3">
        {events.slice(0, 8).map(event => (
          <article key={event.id} className="rounded border border-card p-3">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="rounded-full bg-background px-2 py-1 text-xs font-black capitalize">{event.status}</span>
            </div>
            {Number(event.recovery_amount) > 0 ? <small className="mt-2 block text-success">{formatCurrency(Number(event.recovery_amount))} recovered</small> : null}
          </article>
        ))}
      </div>
    </section>
