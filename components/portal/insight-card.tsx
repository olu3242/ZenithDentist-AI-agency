...
export function InsightCard({ insight }: { insight: InsightSnapshot }) {
  ...
  return (
    <article className={cn("rounded border bg-white p-5 shadow-sm", insight.severity === "critical" ? "border-rust/40" : "border-card")}>
      <div className="flex items-start gap-3">
        <span className="rounded bg-accent/10 p-2 text-accent"><Icon className="h-5 w-5" /></span>
        <div>
