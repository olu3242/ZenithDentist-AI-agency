...
export function LeadPipeline({ leads, bookings }: { leads: Lead[]; bookings: Booking[] }) {
  return (
    <section className="grid gap-3 overflow-x-auto rounded border border-line bg-white p-4 lg:grid-cols-6">
      {stages.map(stage => {
        ...
        return (
          <div key={stage} className="min-w-52 rounded bg-paper p-3">
            ...
            <div className="grid gap-2">
              {stageLeads.slice(0, 5).map(lead => (
                <article key={lead.id} className="rounded border border-line bg-white p-3">
